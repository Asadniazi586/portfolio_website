// Projects.jsx - Performance optimized version
import React, { useEffect, useRef, useState } from 'react';
import { motion } from 'framer-motion';
import { FiExternalLink } from 'react-icons/fi';

// Import performance hooks and utilities
...
const Projects = () => {
  // Use optimized inView hook
  const [ref, inView] = useInViewOptimized({ once: false, threshold: 0.1 });
  ...
  // Optimized scroll update function
  const updateScroll = () => {
  ...
  useOptimizedScroll(updateScroll, [maxTranslate, isMobile]);
      ...
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col">
        <div className="flex-1 flex flex-col justify-center">
          <div className="w-full px-4 sm:px-6 md:px-8 lg:px-10">
            ...
            {/* Title Section - Always visible on mobile */}
            <motion.div
              ref={ref}
              initial={{ opacity: 1, y: 0 }}
              animate={{ 
                opacity: showTitle ? 1 : 1,
                y: showTitle ? 0 : 0
              }}
            ...
            <div className="overflow-visible">
              ...
              >
                {projects.map((project, index) => (
                  ...
                  >
                    {/* Glowing border effect on hover - reduced intensity on low-end devices */}
                    <div className={`absolute -inset-[1px] rounded-2xl bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 ${!isLowEnd ? 'opacity-0 group-hover:opacity-100 transition-all duration-500 blur-xl' : 'opacity-0'}`} />
                    <div className={`absolute -inset-[1px] rounded-2xl bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 ${!isLowEnd ? 'opacity-0 group-hover:opacity-40 transition-all duration-500' : 'opacity-0'}`} />
                    ...
                    <div className="relative h-full bg-gradient-to-br from-gray-900/90 to-gray-800/80 rounded-2xl overflow-hidden border border-gray-700/50 shadow-xl group-hover:border-transparent transition-all duration-500">
                      {/* Card shine effect - disabled on low-end devices */}
                      {!isLowEnd && (
